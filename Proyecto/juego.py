...
class Enemigo(object):
    ...
    def movimiento(self, nivel):
        if(self.movimientos >= 130):
            ...
            self.enemigoX = self.enemigoX + (self.direccion * self.velocidad * nivel)
            self.movimientos += 1 * nivel
...
class SpaceInvaders(object):
    ...
    def levelUp(self):
        pygame.mixer.music.load('assets/nivel.mp3')
        pygame.mixer.music.play(0)
    ...
    def run(self):
        ...
        self.vivo=True
        self.threadDisparosEnemigos = threading.Thread(target = self.disparoEnemigos)
        self.threadDisparosEnemigos.start()
        self.jugar=True
        self.upx_random=0
        ...
        while self.jugar:
            ...
            if self.boss_activo == False:
                ...
                if(len(self.enemigo) == 0):
                    self.levelUp()
                    if(self.nivel%3 == 0 and self.boss_activo == False):
                        self.nivel -=1
                        self.pantalla.fill((255,255,255))
                        self.pantalla.blit(self.fondoNegro, [0,0])
                        self.pantalla.blit(self.fuente.render("Vidas Restantes: {}".format(self.jugador.vidas), -1, (255,255,255)), (300, 300))
